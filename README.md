...
DeepSeek 风格像素虎鲸桌面宠物插件（DSH Desktop Pet）——为 [DeepSeek Harness (DSH)](https://github.com/deepseek-ai/deepseek-harness) 提供常驻小鲸桌宠与语音输入。
...
- 像素虎鲸造型（黑身、白眼斑、白腹、灰鞍斑、分叉尾、胸鳍），与 DeepSeek logo 同风格
- 浏览器内置 Web Speech API 语音识别（Edge=Azure、Chrome=Google），零服务端、零 API Key、零模型下载
- 常驻 DSH Web 页面：按住/单击说话、拖动移动、一键开关、可调大小

## Features / 特性

- 像素虎鲸造型：18x13 像素网格手绘，`crispEdges` 锐利边缘
- 语音输入：按住说话转文字（长按模式），或单击切换识别（单击模式）
- 自定义大小：70-190px 步进调节
- 拖拽移动：位置自动记忆（localStorage）
- 一键开关：关闭后缩为休眠小鲸按钮（可拖拽），单击重新开启
- 识别结果直进输入框：写入 composer 草稿（可编辑），或一键发送
